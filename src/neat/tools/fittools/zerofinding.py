    ...
    def f_polynomial(self):
        if len(self.coef) == 1:
            ...
            def p(x):
                if type(x) == np.ndarray:
                    return np.sum(
                        self.coef[np.newaxis, :]
                        * x[:, np.newaxis] ** powers[np.newaxis, :],
                        1,
                    )
                else:
...
def pf_is_left_vec(p1, p2, point):
    ...
    """
    return (p2[:, 0] - p1[:, 0]) * (point[1] - p1[:, 1]) - (point[0] - p1[:, 0]) * (
        p2[:, 1] - p1[:, 1]
    )
...
def pf_winding_number(point, poly):
    ...
    wn += np.where(pf_is_left_vec(poly[ind, :], poly[ind + 1, :], point) > 0)[0].shape[
        0
    ]
    # down intersections
    ind = np.where(np.logical_and(poly[:-1, 1] > point[1], poly[1:, 1] <= point[1]))[0]
...
class contour(object):
    def __init__(self, N_eval=1e3):
        self.curves = [lambda x: np.exp(2.0 * np.pi * 1j * x)]
        self.dcurves = [lambda x: 2.0 * np.pi * 1j * np.exp(2.0 * np.pi * 1j * x)]
        self.N_eval = [N_eval]
        self.make_arrays = False

    def construct_arrays(self):
        self.ts = [np.linspace(0.0, 1.0, int(N)) for N in self.N_eval]
    ...
    def calc_boundaries(self):
        ...
        for i, N in enumerate(self.N_eval):
            if self.make_arrays:
                ...
                c = self.curves[i](np.linspace(0.0, 1.0, int(N)))
            mincr = np.min(c.real)
            maxcr = np.max(c.real)
            if mincr < minreal:
                minreal = mincr
        ...
        if "polygon" not in self.__dict__:
            if self.make_arrays:
                ...
                polygon = np.concatenate(
                    [
                        curve(np.linspace(0.0, 1.0, int(self.N_eval[i])))
                        for i, curve in enumerate(self.curves)
                    ]
                )
            self.polygon = np.concatenate(
                (polygon.real[:, np.newaxis], polygon.imag[:, np.newaxis]), 1
            )
    ...
    def is_inside(self, cpoint):
        ...
        return (
            pf_winding_number(np.array([cpoint.real, cpoint.imag]), self.polygon) != 0
        )


class circularContour(contour):
    def __init__(self, radius=1.0, center=0.0 + 0j, N_eval=1e3):
        super().__init__()
        self.center = center
        self.radius = radius
        self.curves = [lambda x: center + radius * np.exp(2.0 * np.pi * 1j * x)]
        self.dcurves = [
            lambda x: 2.0 * np.pi * 1j * radius * np.exp(2.0 * np.pi * 1j * x)
        ]
        self.N_eval = [N_eval]
...
class trapezoidContour(contour):
    def __init__(self, t1, nparam=1e3, N_eval=1e3):
        ...
        self.curves = [
            lambda x, p1=p, p2=self.points[(i + 1) % len(self.points)]: p1
            + x * (p2 - p1)
            for i, p in enumerate(self.points)
        ]
        self.dcurves = [
            lambda x, p1=p, p2=self.points[(i + 1) % len(self.points)]: (p2 - p1)
            for i, p in enumerate(self.points)
        ]
    ...
    def divide_real_axis(self):
        ps = self.points
        prcut = (ps[0].real + ps[1].real) / 2.0
        pccut = ps[0].imag + (ps[1].imag - ps[0].imag) * (prcut - ps[0].real) / (
            ps[1].real - ps[0].real
        )

        return trapezoidContour(t1=(ps[0], prcut + 1j * pccut)), trapezoidContour(
            t1=(prcut + 1j * pccut, ps[1])
        )
...
class poleFinder:
            ...
            if "poles" in list(kwargs.keys()):
                if "pmultiplicities" in list(kwargs.keys()):
                    self.set_contour(
                        ...
                        use_known_zeros=use_known_zeros,
                        poles=kwargs["poles"],
                        pmultiplicities=kwargs["pmultiplicities"],
                    )
                else:
                    self.set_contour(
                        kwargs["contour"],
    ...
    def set_contour(
        ...
        if len(poles) == 0 and self.has_global_poles:
            ...
            self.poles = np.array(
                [
                    (i, pole)
                    for i, pole in enumerate(self.global_poles)
                    if self.contour.is_inside(pole)
                ]
            )
            if len(self.poles) > 0:
                self.pmultiplicities = self.global_pmultiplicities[
                    self.poles[:, 0].real.astype(int)
                ]
        ...
        if use_known_zeros and len(self.global_zeros) > 0:
            ...
            self.zeros = np.array(
                [
                    (i, zero)
                    for i, zero in enumerate(self.global_zeros)
                    if self.contour.is_inside(zero)
                ]
            )
            if len(self.zeros) > 0:
                self.zmultiplicities = self.global_zmultiplicities[
                    self.zeros[:, 0].real.astype(int)
                ]
    ...
    def inner_prod(self, p1, p2, compute_maxpsum=False):
        ...
        if self.make_arrays:
            integrand = [
                p1(curve_arr) * p2(curve_arr) * self.contour.fun_arrs[i]
                for i, curve_arr in enumerate(self.contour.curve_arrs)
            ]
        else:
        ...
        if len(self.inner_contours) > 0:
            ...
            for inner_contour in self.inner_contours:
                if self.make_arrays:
                    inner_integrand = [
                        p1(curve_arr) * p2(curve_arr) * inner_contour.fun_arrs[i]
                        for i, curve_arr in enumerate(inner_contour.curve_arrs)
                    ]
                else:
                    inner_integrand = (
                        lambda s: p1(s) * p2(s) * self.dfun(s) / self.fun(s)
                    )
                inner_sum -= self.contour_integral(
                    fun=inner_integrand, contour=inner_contour
                )
        else:
            inner_sum = 0.0
        ...
        if compute_maxpsum:
            ...
            return (
                self.contour_integral(fun=integrand) + sum_poles + sum_zeros + inner_sum
            )
    ...
    def generalized_hankel_matrices(self, phis, pols):
        ...
        prow = [
            monicPolynomial(
                npol.polymul(pols[1].coef, pols[j].coef), coef_type="normal"
            ).f_polynomial()
    ...
    def contour_integral(self, fun=None, contour=None, compute_maxpsum=False):
        ...
        if self.make_arrays:
            ...
            for i, t in enumerate(contour.ts):
                ...
                dt = t[1] - t[0]
                if compute_maxpsum:
                    cumpsum = psum + dt * np.cumsum(fval[:-1] + fval[1:]) / (
                        4.0 * np.pi * 1j
                    )
                    psum = cumpsum[-1]
                    ...
                    psum += dt * np.sum(fval[:-1] + fval[1:]) / (4.0 * np.pi * 1j)
        else:
            if fun == None:
            ...
            for i, curve in enumerate(contour.curves):
                ...
                dt = 1.0 / contour.N_eval[i]
                if isinstance(fun, types.FunctionType):
                    t = np.linspace(0.0, 1.0, int(contour.N_eval[i]))
    ...
    def check_for_nonnumeric(self):
        ...
        if self.make_arrays:
            ...
            ts = [np.linspace(0.0, 1.0, int(N)) for N in self.countour.N_eval]
            fun_arrs = [self.dfun(t) / self.fun(t) for t in ts]

        contains_nan = [np.isnan(fun_arrs[ii]).any() for ii in range(len(fun_arrs))]
        contains_inf = [
            not np.isfinite(fun_arrs[ii]).all() for ii in range(len(fun_arrs))
        ]
    ...
    def find_zeros(self, pprint=False):
        ...
        if N == 0:
            ...
            p_aux = monicPolynomial([0.0]).f_polynomial()
            ...
            while r + t < N:
                ...
                prod_aux, maxpsum = self.inner_prod(
                    phis[-1], phis[-1], compute_maxpsum=True
                )
                ...
                if self.points_within_contour(eigv + mu):
                    ...
                    pol = monicPolynomial(
                        npol.polymul(c, pols[-1].coef), coef_type="normal"
                    )
                    pols.append(pol)
                    phis.append(pol.f_polynomial())
                    t += 1
    ...
    def find_real_zeros_recursively(
        self,
        realtol=1e-4,
        minradius=1.0,
        depth=0,
        maxdepth=10,
        contourparams_orig=None,
        pprint=False,
    ):
        """
        ...
        if self.test_contour(pprint=pprint):
            ...
            if len(realzero_inds) == 0 and len(zeros) % 2 == 1:
                ...
                zeros = self._continue_real_zero_recursion(
                    depth=depth,
                    maxdepth=maxdepth,
                    contourparams_orig=contourparams_orig,
                )
            else:
            ...
            if contourparams_orig != None:
                contour = circularContour(
                    center=contourparams_orig[0], radius=contourparams_orig[1]
                )
            ...
            zeros = self._continue_real_zero_recursion(
                depth=depth, maxdepth=maxdepth, contourparams_orig=contourparams_orig
            )
            # restore the original contour
            if contourparams_orig != None:
                contour = circularContour(
    ...
    def _continue_real_zero_recursion(
        self,
        realtol=1e-4,
        minradius=1.0,
        depth=0,
        ...
        c0 = self.contour.center.real
        r0 = self.contour.radius
        if r0 > minradius and depth < maxdepth:
            # first new contour for zero finding
            contour1 = circularContour(center=c0 - r0 / 2.0 + 0j, radius=r0 / 2.0)
            self.set_contour(contour1, make_arrays=True)
            zeros1 = self.find_real_zeros_recursively(
                realtol=realtol,
                minradius=minradius,
                depth=depth,
                maxdepth=maxdepth,
            ...
            contour2 = circularContour(center=c0 + r0 / 2.0 + 0j, radius=r0 / 2.0)
            self.set_contour(contour2, make_arrays=True)
            zeros2 = self.find_real_zeros_recursively(
                realtol=realtol,
                minradius=minradius,
                depth=depth,
                maxdepth=maxdepth,
    ...
    def find_zeros_and_poles_(
        self, eps_reg=1e-15, eps_stop=1e-10, P_estimate=0, pprint=False
    ):
        """
        Find the zeros and poles of a complex function (C->C) inside a closed curve.
            ...
            if self.points_within_contour(eigv + mu):
                ...
                while allsmall and (r + tau) < N:
                    ...
                    tauphi = monicPolynomial(
                        npol.polymul(taupol, pols[-1].coef), coef_type="normal"
                    ).f_polynomial()
    ...
    def find_real_zeros(self, xtol=1e-9, vmin=None, vmax=None):
        ...
        for i in range(len(plist) - 1):
            ...
            if xmin < xmax:
                ...
                sf0 = np.sign(f_aux(xmin))
                sf1 = np.sign(f_aux(xmax))
                sdf0 = np.sign(df_aux(xmin))
                sdf1 = np.sign(df_aux(xmax))
                xs = []
                if sf0 != sf1:
...
def find_zeros_on_segment(
    ...
    cc = circularContour(
        radius=(xmax - xmin) / 2.0, center=(xmax + xmin) / 2.0 + 0j, N_eval=1e2
    )
    PF = poleFinder(
        fun=fun,
        dfun=dfun,
        global_poles={"poles": poles, "pmultiplicities": pmultiplicities},
        make_arrays=True,
        use_known_zeros=False,
        contour=cc,
    )
    ...
    if pprint:
        ...
        print("radius = ", (xmax - xmin) / 2.0, ", center = ", (xmax + xmin) / 2.0)
        print(PF.test_contour(pprint=pprint))

    if PF.test_contour():
        # find the zero
        p1, info = PF.find_zeros(pprint=False)
        zeros.extend(p1.tolist())
        zmultiplicities.extend(info[1].tolist())
    else:
        xmin0 = xmin
        xmax0 = (xmax + xmin) / 2.0
        xmin1 = xmax0
