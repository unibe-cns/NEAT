...
template <>
void
DynamicRecordablesMap< multichannel_test_model >::create( multichannel_test_model& host )
{
...
nest::multichannel_test_model::multichannel_test_model()
  : ArchivingNode()
  , c_tree_()
  , syn_buffers_( 0 )
  , logger_( *this )
...
nest::multichannel_test_model::multichannel_test_model( const multichannel_test_model& n )
  : ArchivingNode( n )
  , c_tree_( n.c_tree_ )
 ...
 */
void
multichannel_test_model::get_status( DictionaryDatum& statusdict )
{
  ...
  for ( long comp_idx_ = 0; comp_idx_ != c_tree_.get_size(); comp_idx_++ )
  {
    ...
    CompartmentMultichannelTestModel* compartment = c_tree_.get_compartment( comp_idx_ );
...
void
nest::multichannel_test_model::set_status( const DictionaryDatum& statusdict )
{
...
void
nest::multichannel_test_model::add_compartment_( DictionaryDatum& dd )
{
...
void
nest::multichannel_test_model::add_receptor_( DictionaryDatum& dd )
{
...
void
nest::multichannel_test_model::init_recordables_pointers_()
{
  /**
  ...
  for ( auto rec_it = recordables.begin(); rec_it != recordables.end(); rec_it++ )
  {
    ...
    if ( recname_it == recordables_names.end() )
    {
      ...
      recordablesMap_.insert( rec_it->first, DataAccessFunctor< multichannel_test_model >( *this, rec_idx ) );
    }
    else
    {
...
void
nest::multichannel_test_model::pre_run_hook()
{
  logger_.init();
 ...
 */
void
nest::multichannel_test_model::update( Time const& origin, const long from, const long to )
{
...
void
nest::multichannel_test_model::handle( SpikeEvent& e )
{
  if ( e.get_weight() < 0 )
  {
...
void
nest::multichannel_test_model::handle( CurrentEvent& e )
{
/* REPLACED ONLY FOR TESTING OF CONTINUOUS INPUT
  assert( e.get_delay_steps() > 0 );

  const double c = e.get_current();
  const double w = e.get_weight();

  CompartmentMultichannelTestModel* compartment = c_tree_.get_compartment_opt( e.get_rport() );
...
void
nest::multichannel_test_model::handle( DataLoggingRequest& e )
{
  logger_.handle( e );
}
