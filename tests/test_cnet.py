...
class TestCNET():
    def create_point_neurons(self, v_eq=-75.):
        ...
        # add the synapse
        # to neuron tree
        self.sim_tree.add_double_exp_synapse((1,.5), .2, 3., 0.)
        self.sim_tree.set_spiketrain(0, 0.001, [5.])
        # to net sim
    ...
    def test_solver(self):
        ...
        v_loc_middle_1 = self.cnet.solve_newton(g_list, v_0=np.array([0.,0.,0.]),
    ...
    def test_integration(self):
        ...
        # add spike times
        self.cnet.set_spiketimes(0, np.arange(dt/2., tmax, dt))
        # run sim
        res = self.cnet.run_sim(tmax, dt, step_skip=1, rec_v_node=True,
                                                      rec_g_syn_inds=[0])
        ...
        self.cnet.add_synapse(1, "AMPA", g_max=dt*0.1)
        # add spike times
        self.cnet.set_spiketimes(0, np.arange(dt/2., tmax, dt)) # GABA synapse
        self.cnet.set_spiketimes(1, np.arange(dt/2., tmax, dt)) # AMPA synapse
        # run sim
        res = self.cnet.run_sim(tmax, dt, step_skip=1, rec_v_node=True,
                                                      rec_g_syn_inds=[0,1])
        v_loc_sim = res['v_loc'][:,-1]
        ...
        # add NMDA synapse
        self.cnet.add_synapse(0, "AMPA+NMDA", g_max=dt*0.1, nmda_ratio=5.)
        self.cnet.add_synapse(1, "AMPA+NMDA", g_max=dt*0.1, nmda_ratio=5.)
        # set spiketimes for second synapse
        self.cnet.set_spiketimes(3, np.arange(dt/2., tmax, dt)) # AMPA+NMDA synapse
        ...
        # set spiketimes
        self.cnet.set_spiketimes(0, np.array([5.]))
        self.cnet.set_spiketimes(1, np.array([10.]))
        # run sim
        res1 = self.cnet.run_sim(tmax, dt, step_skip=1, rec_v_node=True,
                                                      rec_g_syn_inds=[0,1,2])
        # set spiketimes
        self.cnet.set_spiketimes(0, np.array([5.]))
        self.cnet.set_spiketimes(1, np.array([10.]))
        # run sim
        res2 = self.cnet.run_sim(tmax, dt, step_skip=ss, rec_v_node=True,
                                                      rec_g_syn_inds=[0,1,2])
    ...
    def test_inversion(self):
        ...
        # construct inputs
        g_in = self.cnet.recast_input([1.,1.,1.])
