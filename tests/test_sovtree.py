...
class TestSOVTree():
    ...
    def testSingleCompartment(self):
        ...
        greenstree.set_impedance(np.array([0.]))
        z_inp = greenstree.calc_impedance_matrix([(1.,0.5)])

        self.btree.calc_sov_equations(maxspace_freq=500)
        alphas, gammas = self.btree.get_sov_matrices(loc_arg=[(1.,.5)])
        z_inp_sov = self.btree.calc_impedance_matrix(loc_arg=[(1.,.5)])
    ...
    def testNETDerivation(self):
        ...
        net, lin_terms = self.tree.construct_net(dz=20., add_lin_terms=True)
        # check if correct
        alphas, gammas = self.tree.get_important_modes(loc_arg='net eval',
